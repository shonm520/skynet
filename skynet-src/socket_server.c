...
new_fd(struct socket_server *ss, int id, int fd, int protocol, uintptr_t opaque, bool add) {
	...
	s->opaque = opaque;   //opaque指src服务的id
	s->wb_size = 0;
	s->warn_size = 0;
...
listen_socket(struct socket_server *ss, struct request_listen * request, struct socket_message *result) {
	int id = request->id;           //id是累加上去的
	int listen_fd = request->fd;    //fd是do_listen产生的
...
_failed:
	close(listen_fd);
	result->opaque = request->opaque;    //opaque指src服务的id
	result->id = id;
	result->ud = 0;
	result->data = "reach skynet socket number limit";
	ss->slot[HASH_ID(id)].type = SOCKET_TYPE_INVALID;
...
ctrl_cmd(struct socket_server *ss, struct socket_message *result) {
	...
	uint8_t buffer[256];
	uint8_t header[2];
	block_readpipe(fd, header, sizeof(header));      //先读取头两个字节:type,len
	int type = header[0];
	int len = header[1];
	block_readpipe(fd, buffer, len);    //再读取len长度数据
...
socket_server_listen(struct socket_server *ss, uintptr_t opaque, const char * addr, int port, int backlog) {
	...
	send_request(ss, &request, 'L', sizeof(request.u.listen));     //发送给管道的数据为1字节类型，1字节长度，len字节的数据，共len+2字节
	return id;
}
